...
const areacodes = await import('../data/areacodes.js');
...
class OpenTrace {
  ...
  isAddress(val) {
    /* 
     ...
     it('should return true for a valid physical address', () => {
     const validAddress = "123 Main St, City, Country";
     assert.isTrue(isAddress(validAddress));
   });
 
   it('should return false for an empty string', () => {
     const emptyAddress = "";
     assert.isFalse(isAddress(emptyAddress));
   });
 
   it('should return false for an address with special characters', () => {
     const specialCharsAddress = "Address with @ symbols";
     assert.isFalse(isAddress(specialCharsAddress));
   });
 
   it('should return true for an address with numbers and letters', () => {
     const alphanumericAddress = "42 Elm Avenue";
     assert.isTrue(isAddress(alphanumericAddress));
   });
 
   it('should return true for an address with hyphens and spaces', () => {
     const hyphenatedAddress = "Apt 3B - 789 Oak St";
     assert.isTrue(isAddress(hyphenatedAddress));
   });
 
   it('should return true for an address with international characters', () => {
     const internationalAddress = "123 Rue de l'Étoile, Paris";
     assert.isTrue(isAddress(internationalAddress));
   });
 
   it('should return false for an address with excessive spaces', () => {
     const excessiveSpacesAddress = "   456  White     Birch   Road   ";
     assert.isFalse(isAddress(excessiveSpacesAddress));
   });
    */
